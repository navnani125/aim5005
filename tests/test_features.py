...
class TestFeatures(TestCase):
    ...
    def test_label_encoder_fit(self):
        ...
        assert (encoder.classes_ == expected_classes).all(), "LabelEncoder fit does not return expected classes."
        
    def test_label_encoder_transform(self):
        encoder = LabelEncoder()
        data = ["cat", "dog", "cat", "bird"]
        encoder.fit(data)
        result = encoder.transform(["cat", "dog", "bird", "cat"])
        expected = np.array([1, 2, 0, 1])  # Assuming 'bird' -> 0, 'cat' -> 1, 'dog' -> 2
        assert (result == expected).all(), "LabelEncoder transform does not return expected values. Expect {}. Got: {}".format(expected, result)
        
    def test_label_encoder_fit_transform(self):
        encoder = LabelEncoder()
        data = ["cat", "dog", "cat", "bird"]
        result = encoder.fit_transform(data)
        expected = np.array([1, 2, 1, 0])  # Based on the previous assumptions
        assert (result == expected).all(), "LabelEncoder fit_transform does not return expected values. Expect {}. Got: {}".format(expected, result)
